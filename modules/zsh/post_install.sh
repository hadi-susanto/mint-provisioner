...
if [[ ! -f "$TARGET_PAYLOAD" ]] || [[ "$ZSH_FORCE_CONFIGURATION" == "true" ]]; then
    log_info "[$MODULE] Copying configuration file $PAYLOAD_FILE to $TARGET_PAYLOAD (configuration)"
    cp "$SCRIPT_DIR/payload/$PAYLOAD_FILE" "$TARGET_PAYLOAD"
else
    log_warn "[$MODULE] $TARGET_PAYLOAD already exists and ZSH_FORCE_CONFIGURATION is not true, skipping"
fi
...
if ! grep -Fq "$SOURCE_LINE" "$ZSHRC_FILE"; then
    ...
    echo "$SOURCE_LINE" | tee -a "$ZSHRC_FILE" > /dev/null
else
