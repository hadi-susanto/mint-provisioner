#!/usr/bin/env bash

source "${LIB_DIR}/installer_common.sh"

MODULE="delta"
STATE_FILE="${STATE_DIR}/delta.path"

if [[ ! -f "$STATE_FILE" ]]; then
    log_error "[$MODULE] State file not found: $STATE_FILE"

    exit 1
fi

read -r ARCHIVE_FILE < "$STATE_FILE"

if [[ ! -f "$ARCHIVE_FILE" ]]; then
    log_error "[$MODULE] Archive file not found: ${ARCHIVE_FILE}"

    exit 2
fi

if [[ -z "${DELTA_INSTALL_DIR:-}" ]]; then
    DELTA_INSTALL_DIR="$INSTALL_DIR/delta"
fi

SUDO_CMD=""
if ! can_write "$(dirname "$DELTA_INSTALL_DIR")"; then
    SUDO_CMD="sudo"
fi

if ! $SUDO_CMD mkdir -p "$DELTA_INSTALL_DIR"; then
    log_error "[$MODULE] Failed to create install directory: $DELTA_INSTALL_DIR"

    exit 3
fi

if ! $SUDO_CMD tar --overwrite -xzf "$ARCHIVE_FILE" -C "$DELTA_INSTALL_DIR" --strip-components=1; then
    log_error "[$MODULE] Extraction failed"

    exit 4
fi

if ! $SUDO_CMD chmod +x "$DELTA_INSTALL_DIR/delta"; then
    log_error "[$MODULE] Failed to make binary executable"

    exit 5
fi

log_info "[$MODULE] Creating symbolic links"
if [[ -f "$DELTA_INSTALL_DIR/delta" ]]; then
    symlink_binary "$MODULE" "$DELTA_INSTALL_DIR/delta"
else
    log_error "[$MODULE] Binary not found at $DELTA_INSTALL_DIR/delta"
fi

log_info "[$MODULE] Installation completed successfully"
