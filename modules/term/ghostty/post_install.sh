...
log_info "[$CANONICAL_ID] Configuring ghostty default behavior based on mint-provisioner"

# 1. Create ghostty config directory if it doesn't exist
if [[ ! -d "$GHOSTTY_CONFIG_DIR" ]]; then
    log_info "[$CANONICAL_ID] Creating directory $GHOSTTY_CONFIG_DIR"
    mkdir -p "$GHOSTTY_CONFIG_DIR"
fi

# 2. Copy payload files to ~/.config/ghostty/
log_info "[$CANONICAL_ID] Copying configuration files to $GHOSTTY_CONFIG_DIR"

FILE="mint-provisioner.ghostty"
TARGET="$GHOSTTY_CONFIG_DIR/$FILE"

if [[ ! -f "$TARGET" ]] || [[ "$GHOSTTY_FORCE_CONFIGURATION" == "true" ]]; then
    log_info "[$CANONICAL_ID] Copying $FILE to $GHOSTTY_CONFIG_DIR"
    cp "$PAYLOAD_DIR/$FILE" "$TARGET"
else
    log_warn "[$CANONICAL_ID] $FILE already exists and GHOSTTY_FORCE_CONFIGURATION is not true, skipping"
fi
...
if [[ ! -f "$GHOSTTY_CONF" ]]; then
    log_info "[$CANONICAL_ID] Creating $GHOSTTY_CONF"
    touch "$GHOSTTY_CONF"
fi

INCLUDE_LINE="config-file=\"$FILE\""
if ! grep -Fxq "$INCLUDE_LINE" "$GHOSTTY_CONF"; then
    log_info "[$CANONICAL_ID] Adding '$INCLUDE_LINE' to $GHOSTTY_CONF"
    echo "$INCLUDE_LINE" >> "$GHOSTTY_CONF"
else
    log_warn "[$CANONICAL_ID] '$INCLUDE_LINE' already exists in $GHOSTTY_CONF"
fi

log_info "[$CANONICAL_ID] ghostty terminal should be configured based on mint-provisioner"
