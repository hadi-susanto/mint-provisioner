...
THEMES_INSTALL_DIR="${OH_MY_POSH_THEMES_INSTALL_DIR:-$OH_MY_POSH_INSTALL_DIR/themes}"

log_info "[$MODULE] Installing themes to $THEMES_INSTALL_DIR"

SUDO_CMD=""
if ! can_write "$THEMES_INSTALL_DIR"; then
    SUDO_CMD="sudo"
fi

if ! SUDO_CMD mkdir -p "$THEMES_INSTALL_DIR"; then
    log_error "[$MODULE] Failed to create themes directory: $THEMES_INSTALL_DIR"

    exit 6
fi

if ! SUDO_CMD unzip -o "$THEMES_FILE" -d "$THEMES_INSTALL_DIR"; then
